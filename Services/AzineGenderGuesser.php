 ...
 * @author dominik
 */
class AzineGenderGuesser implements GenderGuesser
{
    private $debug = 0;
    ...
    public function __construct($femaleNames = null, $malesNames = null, $merge = false, $matchList = null)
    {
        ...
        $this->gender_init($femaleNames, $malesNames, $merge);
    }

    private function gender_init($femaleNames = null, $malesNames = null, $merge = false)
    {
        if (null != $femaleNames) {
            if ($merge) {
                $this->females = array_merge($this->females, $femaleNames);
            } else {
                $this->females = $femaleNames;
            }
            arsort($this->females);
        }
        ...
        if (null != $malesNames) {
            if ($merge) {
                $this->males = array_merge($this->males, $malesNames);
            } else {
                $this->males = $malesNames;
            }
            arsort($this->males);
        }
    }
    ...
    public function guess($name, $looseness = 1)
    {
        ...
        for ($i = 0; $i < $looseness; ++$i) {
            ...
            if ($this->debug) {
                $this->debugMsg .= "\t{$this->MATCH_LIST[$i]}...\n";
            }

            $functionName = $this->MATCH_LIST[$i];
            $gender = $this->$functionName($name);

            if ($this->debug && isset($gender)) {
                $this->debugMsg .= "\t==> HIT ($gender)\n";
            }
    ...
    private function one_only($name)
    {
        ...
        // Match one list only
        $maleHit = isset($this->males[$name]);
        $femaleHit = isset($this->females[$name]);
    ...
    private function either_weight($name)
    {
        ...
        $maleHit = isset($this->males[$name]) ? $this->males[$name] : 0;
        $femaleHit = isset($this->females[$name]) ? $this->females[$name] : 0;

        if (($femaleHit > 0) || ($maleHit > 0)) {
            $gender = ($femaleHit > $maleHit) ? 'f' : 'm';
        }

        if ($this->debug && isset($gender)) {
            $this->debugMsg .= "\tF: $femaleHit, M: $maleHit\n";
        }
    ...
    private function v2_rules($name)
    {
        ...
        // jon and john
        if (preg_match('/^joh?n/', $name)) {
            $gender = 'm';
        }
        // tom and thomas and tomas and toby
        elseif (preg_match('/^th?o(m|b)/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^frank/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^bil/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^hans/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^ron/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^ro(z|s)/', $name)) {
            $gender = 'f';
        } elseif (preg_match('/^walt/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^krishna/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^tri(c|sh)/', $name)) {
            $gender = 'f';
        }
        // pascal and pasqual
        elseif (preg_match('/^pas(c|qu)al$/', $name)) {
            $gender = 'm';
        } elseif (preg_match('/^ellie/', $name)) {
            $gender = 'f';
        } elseif (preg_match('/^anfernee/', $name)) {
            $gender = 'm';
        }
    ...
    private function v1_rules($name)
    {
        ...
        if (preg_match('/^.*[aeiy]$/', $name)) {
            $gender = 'f';
        }
        // allison and variations
        if (preg_match('/^all?[iy]((ss?)|z)on$/', $name)) {
            $gender = 'f';
        }
        // cathleen, eileen, maureen
        if (preg_match('/een$/', $name)) {
            $gender = 'f';
        }
        // barry, larry, perry
        if (preg_match('/^[^s].*r[rv]e?y?$/', $name)) {
            $gender = 'm';
        }
        // clive, dave, steve
        if (preg_match('/^[^g].*v[ei]$/', $name)) {
            $gender = 'm';
        }
        // carolyn, gwendolyn, vivian
        if (preg_match('/^[^bd].*(b[iy]|y|via)nn?$/', $name)) {
            $gender = 'f';
        }
        // dewey, stanley, wesley
        if (preg_match('/^[^ajklmnp][^o][^eit]*([glrsw]ey|lie)$/', $name)) {
            $gender = 'm';
        }
        // heather, ruth, velvet
        if (preg_match('/^[^gksw].*(th|lv)(e[rt])?$/', $name)) {
            $gender = 'f';
        }
        // gregory, jeremy, zachary
        if (preg_match('/^[cgjwz][^o][^dnt]*y$/', $name)) {
            $gender = 'm';
        }
        // leroy, murray, roy
        if (preg_match('/^.*[rlr][abo]y$/', $name)) {
            $gender = 'm';
        }
        // abigail, jill, lillian
        if (preg_match('/^[aehjl].*il.*$/', $name)) {
            $gender = 'f';
        }
        // janet, jennifer, joan
        if (preg_match('/^.*[jj](o|o?[ae]a?n.*)$/', $name)) {
            $gender = 'f';
        }
        // duane, eugene, rene
        if (preg_match('/^.*[grguw][ae]y?ne$/', $name)) {
            $gender = 'm';
        }
        // fleur, lauren, muriel
        if (preg_match('/^[flm].*ur(.*[^eotuy])?$/', $name)) {
            $gender = 'f';
        }
        // lance, quincy, vince
        if (preg_match('/^[clmqtv].*[^dl][in]c.*[ey]$/', $name)) {
            $gender = 'm';
        }
        // margaret, marylou, miri;
        if (preg_match('/^m[aei]r[^tv].*([^cklnos]|([^o]n))$/', $name)) {
            $gender = 'f';
        }
        // clyde, kyle, pascale
        if (preg_match('/^.*[ay][dl]e$/', $name)) {
            $gender = 'm';
        }
        // blake, luke, mi;
        if (preg_match('/^[^o]*ke$/', $name)) {
            $gender = 'm';
        }
        // carol, karen, shar;
        if (preg_match('/^[cks]h?(ar[^lst]|ry).+$/', $name)) {
            $gender = 'f';
        }
        // pam, pearl, rachel
        if (preg_match('/^[pr]e?a([^dfju]|qu)*[lm]$/', $name)) {
            $gender = 'f';
        }
        // annacarol, leann, ruthann
        if (preg_match('/^.*[aa]nn.*$/', $name)) {
            $gender = 'f';
        }
        // deborah, leah, sarah
        if (preg_match('/^.*[^cio]ag?h$/', $name)) {
            $gender = 'f';
        }
        // frances, megan, susan
        if (preg_match('/^[^ek].*[grsz]h?an(ces)?$/', $name)) {
            $gender = 'f';
        }
        // ethel, helen, gretchen
        if (preg_match('/^[^p]*([hh]e|[ee][lt])[^s]*[ey].*[^t]$/', $name)) {
            $gender = 'f';
        }
        // george, joshua, theodore
        if (preg_match('/^[^el].*o(rg?|sh?)?(e|ua)$/', $name)) {
            $gender = 'm';
        }
        // delores, doris, precious
        if (preg_match('/^[dp][eo]?[lr].*s$/', $name)) {
            $gender = 'f';
        }
        // anthony, henry, rodney
        if (preg_match('/^[^jpswz].*[denor]n.*y$/', $name)) {
            $gender = 'm';
        }
        // karin, kim, kristin
        if (preg_match('/^k[^v]*i.*[mns]$/', $name)) {
            $gender = 'f';
        }
        // bradley, brady, bruce
        if (preg_match('/^br[aou][cd].*[ey]$/', $name)) {
            $gender = 'm';
        }
        // agnes, alexis, glynis
        if (preg_match('/^[acgk].*[deinx][^aor]s$/', $name)) {
            $gender = 'f';
        }
        // ignace, lee, wallace
        if (preg_match('/^[ilw][aeg][^ir]*e$/', $name)) {
            $gender = 'm';
        }
        // juliet, mildred, millicent
        if (preg_match('/^[^agw][iu][gl].*[drt]$/', $name)) {
            $gender = 'f';
        }
        // ari, bela, ira
        if (preg_match('/^[abeiuy][euz]?[blr][aeiy]$/', $name)) {
            $gender = 'm';
        }
        // iris, lois, phyllis
        if (preg_match('/^[egilp][^eu]*i[ds]$/', $name)) {
            $gender = 'f';
        }
        // randy, timothy, tony
        if (preg_match('/^[art][^r]*[dhn]e?y$/', $name)) {
            $gender = 'm';
        }
        // beatriz, bridget, harriet
        if (preg_match('/^[bhl].*i.*[rtxz]$/', $name)) {
            $gender = 'f';
        }
        // antoine, jerome, tyrone
        if (preg_match('/^.*oi?[mn]e$/', $name)) {
            $gender = 'm';
        }
        // danny, demetri, dondi
        if (preg_match('/^d.*[mnw].*[iy]$/', $name)) {
            $gender = 'm';
        }
        // pete, serge, shane
        if (preg_match('/^[^bg](e[rst]|ha)[^il]*e$/', $name)) {
            $gender = 'm';
        }
        // angel, gail, isabel
        if (preg_match('/^[adfgim][^r]*([bg]e[lr]|il|wn)$/', $name)) {
            $gender = 'f';
        }
